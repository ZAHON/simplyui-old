...
function NativeSelectTest(props: Partial<NativeSelectProps>) {
  return (
    <NativeSelect {...props}>
...
describe('NativeSelect', () => {
  ...
  describe('Right section wrapper element', () => {
    ...
    it('should contain element provided by rightSection property', () => {
      const rightSectionContent = 'Test rightSection';
      const rightSection = <p>{rightSectionContent}</p>;

      const { container } = render(<NativeSelectTest rightSection={rightSection} />);

      // eslint-disable-next-line testing-library/no-container, testing-library/no-node-access
      const iconElement = container.querySelector('svg');
      const rightSectionElement = screen.getByText(rightSectionContent);

      expect(iconElement).not.toBeInTheDocument();
      expect(rightSectionElement).toBeInTheDocument();
    });
  });
  ...
  describe('Icon element', () => {
    ...
    it('should support custom icon size when iconSize property provided as number', () => {
      ...
      const { container } = render(<NativeSelectTest iconSize={iconSize} />);
